...
#
# Deliberately does NOT import app.agents.curator_agent — that module (and
...
# CuratorAgent.RankedArticle / DigestItem instances at runtime (see
# app/services/digest_service.py, the only caller), but this file only
# touches their `.digest_id`/`.rank`/`.relevance_score`/`.reasoning` /
# `.article_type`/`.db_id` attributes — duck-typed, no import needed.
...
class UserRankingRepository(BaseRepository[UserRanking]):
    ...
    def replace_for_user(
        ...
        insert the fresh ranking. /feed always reflects the LATEST digest run,
        not an ever-growing history.

        ranked_scores: List[RankedArticle] (app.agents.curator_agent) — not
        imported here to avoid a circular import, see module docstring.
        item_map: digest_id -> DigestItem (same lookup DigestService already
        builds for EmailAgent), used only to recover content_type/content_id
        from each RankedArticle's digest_id ("<article_type>:<db_id>").
        """
