...
#
# Persists the OUTPUT of CuratorAgent's ranking pass (app/agents/curator_agent.py)
# so the Django web app can render a personalized feed without ever calling the
# LLM-based ranker synchronously from a web request. DigestService writes this
# table once per digest run (see app/services/digest_service.py); web/ only
# ever reads it (via web/apps/catalog/models.py's UserRanking mirror).
#
